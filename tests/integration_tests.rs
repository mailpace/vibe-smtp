...
async fn test_email_with_attachment() -> Result<()> {
    ...
    let result = transport.send(&email);
    assert!(result.is_ok(), "Email with attachment should be sent successfully");
...
async fn test_email_with_mailpace_headers() -> Result<()> {
    ...
    let result = transport.send(&email);
    assert!(result.is_ok(), "Email with MailPace headers should be sent successfully");
...
async fn test_authentication_failure() -> Result<()> {
    ...
    let transport = create_smtp_transport(
        server.smtp_address(),
        Some(Credentials::new("wrong-token".to_string(), "wrong-token".to_string()))
    );
    ...
    assert!(result.is_ok(), "Mock server accepts all tokens in this test");
...
async fn test_multiple_recipients() -> Result<()> {
    ...
    let result = transport.send(&email);
    assert!(result.is_ok(), "Email to multiple recipients should be sent successfully");
...
async fn test_smtp_commands_ehlo() -> Result<()> {
    ...
    response.clear();
    buf_reader.read_line(&mut response).await?;
    assert!(response.contains("250"), "Should receive positive response to EHLO");
